            ...
            for (ach in 1:length(mem.depth.vec) ) {
              for (eye in 1:length(mem.weight.vec) ) {
                ...
                for (ge in 1:replicates){
                  ...
                  sim <- pop.habitatselection(POP.SIZE = 1000,
                  ...
                  fit.local <- glm(moore.out$big.table[,"n.eggs",1] ~ moore.out$big.table[,"risk.score",1], family = poisson(link = 'log'))
                  summary(fit.local)
                  ...
                  directions.table[count, 11:12] <- c(summary(fit.local)$coef[2,1], summary(fit.local)$coef[2,4])
                    ...
                    directions.table[count, 13:14] <- c(summary(fit.regional)$coef[2,1], summary(fit.regional)$coef[2,4])
                      ...
                      safe.fit.local <- glm(moore.out$big.table[SAFE,"n.eggs",1] ~ moore.out$big.table[SAFE,"risk.score",1], family = poisson(link = 'log'))
                      safe.directions[count, 11:12] <- c(summary(safe.fit.local)$coef[2,1], summary(safe.fit.local)$coef[2,4])
                      
                      safe.fit.regional <- glm(moore.out$big.table[SAFE,"n.eggs",2] ~ moore.out$big.table[SAFE,"risk.score",2], family = poisson(link = 'log'))
                      ...
                      risk.fit.local <- glm(moore.out$big.table[RISK,"n.eggs",1] ~ moore.out$big.table[RISK,"risk.score",1], family = poisson(link = 'log'))
                      risk.directions[count, 11:12] <- c(summary(risk.fit.local)$coef[2,1], summary(risk.fit.local)$coef[2,4])
                      
                      risk.fit.regional <- glm(moore.out$big.table[RISK,"n.eggs",2] ~ moore.out$big.table[RISK,"risk.score",2], family = poisson(link = 'log'))
...
plot(boxplot(full.local.estimates)$stats[3,],
     boxplot(full.regional.estimates)$stats[3,],
     xlab = "local medians", ylab = 'regional medians')
